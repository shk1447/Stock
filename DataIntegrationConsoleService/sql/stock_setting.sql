...
-- Dumping data for table datasourcebase.data_analysis: ~6 rows (대략적)
...
-- Dumping data for table datasourcebase.data_collection: ~2 rows (대략적)
...
INSERT INTO `data_collection` (`idx`, `name`, `module_name`, `method_name`, `action_type`, `options`, `schedule`, `status`, `unixtime`) VALUES
	(1, 'Stock', 'Finance.dll', 'StockInformation', 'once', _binary 0x0402000A000000030004005300646179736D6574686F64213230303021686973746F7279, _binary 0x0404001800000003000300630008003F0010003F00656E647374617274696E74657276616C7765656B646179732130323A33352132343A3335210405000500000003000100430002003F0003003F00040003013031323334214D4F4E21545545215745442154485521465249, 'stop', '2016-11-01 11:36:12.350'),
	(2, 'Stock_Real', 'Finance.dll', 'StockInformation', 'schedule', _binary 0x0402000A000000030004002300646179736D6574686F642131217265616C74696D65, _binary 0x0404001800000003000300630008003F0010002801656E647374617274696E74657276616C7765656B646179732131353A33302130393A30302136303030300405000500000003000100430002003F0003003F00040003013031323334214D4F4E21545545215745442154485521465249, 'stop', '2016-11-02 09:57:31.302');
...
-- Dumping data for table datasourcebase.data_view: ~0 rows (대략적)
/*!40000 ALTER TABLE `data_view` DISABLE KEYS */;
INSERT INTO `data_view` (`idx`, `name`, `view_type`, `view_query`, `unixtime`) VALUES
	(1, '현재 주가', 'current', 'SELECT column_get(`rawdata`, \'종목명\' as char) as `종목명`, column_get(`rawdata`, \'종가\' as int) `종가`, column_get(`rawdata`, \'5일_이동평균\' as double) as `5일_이동평균`,  `unixtime` as `수집시간` FROM current_finance;', '2016-11-02 13:58:15.746'),
	(2, '실시간 고가', 'current', 'SELECT column_get(`rawdata`, \'종목명\' as char) as `종목명`, column_get(`rawdata`, \'고가\' as char) as `고가` FROM current_finance;', '2016-11-01 12:13:24.436'),
	(13, '과거 종가', 'past', 'SELECT `종가`,`5일_이동평균`, UNIX_TIMESTAMP(`unixtime`) as unixtime FROM (SELECT AVG(column_get(`rawdata`, \'종가\' as int)) as `종가`, AVG(column_get(`rawdata`, \'5일_이동평균\' as int)) as `5일_이동평균`, `unixtime` FROM past_finance WHERE category=\'000020\' GROUP BY unixtime DESC) as result GROUP BY DATE(unixtime);', '2016-11-02 13:57:46.188');
...
-- Dumping data for table datasourcebase.member: ~0 rows (대략적)
/*!40000 ALTER TABLE `member` DISABLE KEYS */;
INSERT INTO `member` (`idx`, `member_id`, `member_name`, `password`, `privilege`, `email`, `phone_number`, `unixtime`) VALUES
	(1, 'admin', 'admin', 'soul1087', 'super', 'sukan8822@gmail.com', '01057721447', '2016-10-18 11:55:34.608');
