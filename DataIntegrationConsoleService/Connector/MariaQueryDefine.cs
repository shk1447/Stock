...
namespace Connector
{
    public class MariaQueryDefine
    {
        ...
        public const string GetAnalysis = "DROP TABLE IF EXISTS `{category}_temp`;  CREATE TEMPORARY TABLE IF NOT EXISTS `{category}_temp`  (`idx` INTEGER NOT NULL AUTO_INCREMENT PRIMARY KEY,  `category` varchar(50),  `거래량` varchar(50),  `종가` varchar(50),  `단기거래량` decimal(20,2),  `장기거래량` decimal(20,2),  `생명선` decimal(20,2),  `temp` decimal(20,2),  `temp2` decimal(20,2),  `temp3` decimal(20,2),  `unixtime` timestamp(3), INDEX `IDX_temporary` (`unixtime`)) ENGINE=MEMORY  AS SELECT result02.idx, {sampling}(result01.종가) as `종가`, SUM(result01.거래량) as `거래량`, result01.category, result01.종가 - result02.종가 as Gain, CAST(NULL as DOUBLE) as AvgGain, CAST(NULL as DOUBLE) as AvgLoss, result01.unixtime  FROM ( SELECT @rownum:=@rownum+1 as idx, category, 종가, 거래량, unixtime FROM ( SELECT category, COLUMN_GET(rawdata, '종가' as double) as `종가`, COLUMN_GET(rawdata, '거래량' as double) as `거래량`, unixtime  FROM past_stock WHERE category = '{category}' AND (@rownum:=0)=0 {time_range} GROUP BY DATE(unixtime)) temp01 ORDER BY unixtime ASC) as result01,  ( SELECT @rownum:=@rownum+1 as idx, category, 종가, unixtime FROM ( SELECT category, COLUMN_GET(rawdata, '종가' as double) as `종가`, unixtime  FROM past_stock  WHERE category = '{category}' AND (@rownum:=0)=0 {time_range} GROUP BY DATE(unixtime)) as temp02 ORDER BY unixtime ASC) as result02  WHERE result01.idx - 1 = result02.idx  {sampling_query};    DROP TABLE IF EXISTS `start_{category}_temp`;  CREATE TEMPORARY TABLE IF NOT EXISTS `start_{category}_temp`  ENGINE=MEMORY  AS SELECT category, SUM(CASE WHEN Gain >= 0 THEN Gain ELSE 0 END) AS start_gain_sum, SUM(CASE WHEN Gain < 0 THEN ABS(Gain) ELSE 0 END) AS start_loss_sum FROM {category}_temp WHERE idx <= 14 GROUP BY category;  SET @avg_gain = 0;  SET @avg_loss = 0;  UPDATE {category}_temp,start_{category}_temp  SET AvgGain = @avg_gain := if(idx = 14, start_{category}_temp.start_gain_sum, if(idx > 14, CAST(@avg_gain AS DOUBLE) * 13 + if(Gain >= 0, Gain, 0), 0))/14, AvgLoss = @avg_loss := if(idx = 14, start_{category}_temp.start_loss_sum, if(idx > 14, CAST(@avg_loss AS DOUBLE) * 13 + if(Gain < 0, abs(Gain), 0), 0))/14, unixtime = unixtime  WHERE {category}_temp.category = start_{category}_temp.category;    SET @ema_intervals = {short_day};  SET @k = 2 / (1 + @ema_intervals);  SET @prev_ema = 0;  UPDATE `{category}_temp`  SET temp = @prev_ema := (case when `{category}_temp`.idx = 1 then `{category}_temp`.`거래량` else `{category}_temp`.`거래량` * @k + @prev_ema * (1 - @k) end),  `단기거래량` = temp, `unixtime` = `unixtime`;  SET @ema_intervals = {long_day};  SET @k = 2 / (1 + @ema_intervals);  SET @prev_ema = 0;  UPDATE `{category}_temp`  SET temp2 = @prev_ema := (case when `{category}_temp`.idx = 1 then `{category}_temp`.`거래량` else `{category}_temp`.`거래량` * @k + @prev_ema * (1 - @k) end), `장기거래량` = temp2, `unixtime` = `unixtime`;  SET @ema_intervals = 20;SET @k = 2 / (1 + @ema_intervals);  SET @prev_ema = 0;UPDATE `{category}_temp` SET temp3 = @prev_ema := (case when `{category}_temp`.idx = 1 then `{category}_temp`.`종가` else `{category}_temp`.`종가` * @k + @prev_ema * (1 - @k) end), `생명선` = temp3, `unixtime` = `unixtime`;      SELECT idx, category, UNIX_TIMESTAMP(DATE(unixtime)) as `unixtime`, 종가, 거래량, Gain, AvgGain, AvgLoss, AvgGain / AvgLoss as RS, 100 - (100 / (1 + AvgGain / AvgLoss)) as RSI, (`단기거래량` - `장기거래량`) / `단기거래량` * 100 as `VOLUME_OSCILLATOR`, 생명선  FROM {category}_temp ORDER BY unixtime ASC;";
