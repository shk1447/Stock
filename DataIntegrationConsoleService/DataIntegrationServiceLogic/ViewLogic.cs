...
namespace DataIntegrationServiceLogic
{
    public class ViewLogic
    {
        ...
        {
            Dictionary<string, List<double>> 매수정보 = new Dictionary<string, List<double>>();
            ...
            for (var day = prev.Date; day.Date <= current.Date; day = day.AddDays(1))
            {
                foreach (var row in categories)
                {
                    ...
                    var volume_query_builder = new StringBuilder(MariaQueryDefine.GetVolumeOscillator).Append(" WHERE 날짜 <= '" + unixtime + "'");
                    ...
                    if (result["전체상태"].ReadAs<string>() == "하락" && result["현재상태"].ReadAs<string>() == "상승")
                    {
                        if (매수정보.ContainsKey(category))
                        {

                        }
                        else
                        {
                            매수정보.Add(category, new List<double>() { result["V패턴_비율"].ReadAs<double>(), result["VOLUME_SIGNAL"].ReadAs<double>() });
                        }
                    }
                }
            }
