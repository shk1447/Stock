            ...
            if (ev.t == 'multi') {
                ...
                if (ev.links) {
                    for (i=0;i<ev.links.length;i++) {
                        // undo에 대한 link 추가이벤트 발생 by khan
                        if(ev.links[i].target._def.onremovelink) {
                            if(ev.links[i].target._def.onremovelink.call(ev.links[i], ev.links[i].source, ev.links[i].target)) {
                ...
                if (ev.nodes) {
                    for (i=0;i<ev.nodes.length;i++) {
                        // undo에 대한 node 추가이벤트 발생 by khan
                        var possible = false;
                        if(ev.nodes[i]._def.onbeforeadd) {
                            possible = ev.nodes[i]._def.onbeforeadd.call(ev.nodes[i]);
                        } else {
                            possible = true;
                        }
                        if(possible) {
                            ...
                            if(ev.nodes[i]._def.onadd) {
                                ev.nodes[i]._def.onadd.call(ev.nodes[i]);
                            }
                        }
                    }
                }
                if (ev.links) {
                    for (i=0;i<ev.links.length;i++) {
                        // undo에 대한 link 추가이벤트 발생 by khan
                        if(ev.links[i].target._def.onaddlink) {
                            if(ev.links[i].target._def.onaddlink.call(ev.links[i].target, ev.links[i].source, ev.links[i].target)) {
