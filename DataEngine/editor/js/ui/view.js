...
RED.view = (function() {
    ...
    function init() {
        ...
        $("#chart").droppable({
            ...
            drop: function( event, ui ) {
                ...
                // node 추가 이벤트에 대한 수정 작업 중... by khan
                var possible = false;
                if(nn._def.onbeforeadd) {
                    possible = nn._def.onbeforeadd.call(nn);
                } else {
                    possible = true;
                }
                if(!possible) return;
                ...
                if (spliceLink) {
                    ...
                    // 최초 node 추가시에 splice하여 link가 추가될 경우에 대한 이벤트 by khan
                    if(spliceLink.target._def.onremovelink) {
                        if(spliceLink.target._def.onremovelink.call(spliceLink.target, spliceLink.source,spliceLink.target)) {
                    ...
                    historyEvent.links = [];
                    if(nn._def.onaddlink) {
                        if(nn._def.onaddlink.call(nn, spliceLink.source, nn)) {
                            RED.nodes.addLink(link1);
                            historyEvent.links.push(link1);
                        }
                    } else {
                        RED.nodes.addLink(link1);
                        historyEvent.links.push(link1);
                    }
                    if(spliceLink.target._def.onaddlink) {
                        if(spliceLink.target._def.onaddlink.call(nn, nn, spliceLink.target)) {
                            RED.nodes.addLink(link2);
                            historyEvent.links.push(link2);
                        }
                    } else {
                        RED.nodes.addLink(link2);
                        historyEvent.links.push(link2);
        ...
        if (mouse_mode == RED.state.MOVING_ACTIVE) {
            if (moving_set.length > 0) {
                ...
                if (activeSpliceLink) {
                    ...
                    historyEvent.links = [];
                    // add splice link에 대한 이벤트 기능 추가 by khan
                    if(moving_set[0].n._def.onaddlink) {
                        if(moving_set[0].n._def.onaddlink.call(this, spliceLink.source, moving_set[0].n)) {
                            RED.nodes.addLink(link1);
                            historyEvent.links.push(link1);
                        }
                    } else {
                    ...
                    if(spliceLink.target._def.onaddlink) {
                        if(spliceLink.target._def.onaddlink.call(this, moving_set[0].n, spliceLink.target)) {
                            RED.nodes.addLink(link2);
                            historyEvent.links.push(link2);
                        }
                    } else {
    ...
    function deleteSelection() {
        if (moving_set.length > 0 || selected_link != null) {
            ...
            if (selected_link) {
                if(selected_link.target._def.onremovelink) {
                    if(selected_link.target._def.onremovelink.call(selected_link.source,selected_link.source,selected_link.target)) {
        ...
        if (mouse_mode == RED.state.JOINING || mouse_mode == RED.state.QUICK_JOINING) {
            ...
            for (i=0;i<drag_lines.length;i++) {
                if (portType != drag_lines[i].portType && mouseup_node !== drag_lines[i].node) {
                    ...
                    if (!existingLink) {
                        var link = {source: src, sourcePort:src_port, target: dst};
                        if(dst._def.onaddlink) {
                            if(dst._def.onaddlink.call(this, src, dst)) {
                                RED.nodes.addLink(link);
