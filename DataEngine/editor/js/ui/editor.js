...
RED.editor = (function() {
    ...
    function prepareConfigNodeButton(node,property,type,prefix) {
        ...
        button.click(function(e) {
            console.log('config add button event');
    ...
    function updateConfigNodeSelect(name,type,value,prefix) {
        ...
        if (prefix) {
            ...
            if (button.length) {
                ...
                // config node가 등록될 때 single이라는 옵션이 true일 경우에는 한개만 by khan
                if (!(RED.nodes.getType(type).category === "config" && RED.nodes.getType(type).single && select.children().length > 0)) {
                ...
                // semantic ui 활성화...일단은...bootstrap 사이즈가 ... by khan
                //select.dropdown();
            }
        }
    }
