...
RED.nodes = (function() {
    ...
    function removeNode(id) {
        ...
        if (id in configNodes) {
            ...
            node = getNode(id);
            var possible = false;
            if (node && node._def.onbeforeremove) {
                possible = node._def.onbeforeremove.call(node);
            } else {
                possible = true;
            }
            if (node && possible) {
                nodes.splice(nodes.indexOf(node),1);
                removingLinks = links.filter(function(l) { return (l.source === node) || (l.target === node); });
                removingLinks.forEach(function(l) {
                    // node 삭제시 삭제되는 link들에 대한 이벤트 발생 by khan
                    if(l.target._def.onremovelink) {
                        if(l.target._def.onremovelink.call(l.target, l.source, l.target)) {
    ...
    function filterConfigs(filter) {
        ...
        for (var id in configNodes) {
            var config = configNodes[id];
            if (filter.hasOwnProperty("type") && config.type !== filter.type) {
                continue;
            }
            if (filter.hasOwnProperty("id") && config.id !== filter.id) {
                continue;
            }
            result.push(config);
        }
    ...
    function filterNodes(filter) {
        ...
        for (var n=0;n<nodes.length;n++) {
            ...
            if (filter.hasOwnProperty("id") && node.id !== filter.id) {
                continue;
            }
            result.push(node);
        }
